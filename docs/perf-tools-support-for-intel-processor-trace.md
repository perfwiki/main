...
The online [perf Intel PT man page](http://www.man7.org/linux/man-pages/man1/perf-intel-pt.1.html ) is not necessarily the latest version, howeverthe wiki has a copy that may be more up-to-date: [perf Intel PT man page](man-pages.md#latest-man-pages).  Also the man page source in the Linux repository is quite readable: [perf Intel PT man page source](http://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/tools/perf/Documentation/perf-intel-pt.txt)
...
[SATT Software Analyze Trace Tool](http://github.com/intel/satt)
...
- [perf Intel PT man page](http://www.man7.org/linux/man-pages/man1/perf-intel-pt.1.html), [perf Intel PT man page (wiki version)](man-pages.md#latest-man-pages) and
...
PT features, we really need to [download and build](download-and-build.md) that
latest `perf`.
...
[Debug Symbol Packages - Ubuntu Wiki](http://wiki.ubuntu.com/Debug%20Symbol%20Packages).
...
If those do not occur, and there is no trace data loss, then system wide tracing will show only one "trace start" and one "trace end" for each CPU.  Additional "trace start" or "trace end" branches would need another explanation i.e.
SMM, secure enclave or trace data loss.
...
- `--itrace=qbe` to show quicker (less detailed) decoding (q), branches (b) and errors (e)
- `-F+flags` to show branch flags such as `tr strt` and `tr end`
- `--ns` to show the timestamp to nanoseconds instead of the default microseconds
...
- `-e` to select which events, i.e. the following:
- `intel_pt/cyc/u` to get Intel PT with cycle-accurate mode, user space only.
- `--filter 'filter f @ ./rdtsc-vs-intel-pt' ` specifies an address filter to trace only f()
...
"psb" is the Intel PT synchronization event.
"cbr" (core-to-bus ratio) shows the CPU frequency.
The "branches" events show the 6 invocations of f() as:
...
Branches are annotated with IPC (instructions-per-cpu-cycle) information.  The number of instructions is always 7.  The number of CPU cycles varies.
Note that the frequency of CPU cycles changes whereas the TSC frequency (2712 MHz in this case) is constant.
...
In general it is not possible to measure the time of an individual instruction with Intel PT,
but with some changes to the program code and using the PTWRITE instruction, it can be done.
...
The reason for using PTWRITE is that it produces a CYC-eligible Intel PT packet
which means we know the cycle count at that point.  Sandwich an instruction
between 2 PTWRITE instructions and we can see how long that instruction took.

Let's measure the time of xsave and xrestore instructions in the Linux kernel.

First patch the kernel to insert PTWRITE instruction's in the right places.
For this, the instruction used is "ptwrite (%rsp)" because it will always work
and we don't care what the PTWRITE payload value is.
...
- `--kcore` to copy kernel object code from the /proc/kcore image (helps avoid decoding errors due to kernel self-modifying code)
- `-e` to select which events, i.e. the following:
- `intel_pt/cyc,ptw,fup_on_ptw/k` to get Intel PT with cycle-accurate mode, PTWRITE+FUP, kernel space only.
- `sh -c 'for i in `seq 1 10`;do sleep 0.01;done' ` is the workload.
```sh
...
- `--kcore` to copy kernel object code from the /proc/kcore image (helps avoid decoding errors due to kernel self-modifying code)
- `-e` to select which events, i.e. the following:
- `intel_pt/cyc,noretcomp,mtc_period=9/k` to get Intel PT with cycle-accurate mode. We add `noretcomp` to get a Intel PT TIP packet from RET instructions, which has the side-effect of also getting a CYC timing packet, and consequently enables calculating a timestamp at that point.  The `mtc_period` is set as high as possible because MTC packets are collected continuously which bloats the trace.
- `--filter 'filter __schedule' ` specifies an address filter to trace only __schedule()
...
- `-e` to select which events, i.e. the following:
- `intel_pt/event,branch=0/u` to get Intel PT Event Trace and disable branch tracing, user space only.
...
- `--ns` to show the timestamp to nanoseconds instead of the default microseconds
The output shows INTR for interrupts, exceptions, faults, and NMI.  In this case the PFA (Page Fault Linear Address) indicates that they are page faults.  IRET indicates a return to user space.
...
The answer is that some JIT compilers like OpenJDK Java provide an interface to record the JIT-compiled code.  For Java it is called Java™ Virtual Machine Tool Interface (JVM TI).  `perf` provides library <u>libperf-jvmti.so</u> to make use of the interface.  To make it work:

- java option `-agentpath` is used to tell java to use <u>libperf-jvmti.so</u> which produces a **jitdump file**
- `perf inject` must be used to inject information from the **jitdump file** into the output perf.data file
...
Refer to the [perf dlfilter](man-pages.md#latest-man-pages) man page for details.
...
but at the moment just prints a disassembly.  The [Intel X86 Encoder Decoder (XED)](http://intelxed.github.io/) is used.  Refer to [Example: Tracing your own code : Hello World](#example-tracing-your-own-code-hello-world)
for how to install XED.
...
Generally, processors with the same CPU core microarchitecture can be expected to have the same Intel PT capabilities.  The Linux Intel PT driver provides capability flags in sysfs, refer [Intel PT man page](#intel-pt-man-page)
